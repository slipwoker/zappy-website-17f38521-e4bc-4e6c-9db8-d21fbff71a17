document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getFieldError(field) {
      ...
      if (field.hasAttribute('minlength') && type !== 'textarea') {
        var minLen = parseInt(field.getAttribute('minlength'), 10);
        if (value.length > 0 && value.length < minLen) {
          return (field.dataset.errorMinlength || 'Minimum {min} characters required.').replace('{min}', minLen);
        }
      }
  ...
  function initScrollAnimations() {
    var elements = document.querySelectorAll(animationSelectors.join(', '));
    ...
    animationObserver = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          var el    = entry.target;
          var delay = el.dataset.delay || el.dataset.animateDelay || '0';
    ...
    Array.prototype.forEach.call(elements, function (el, index) {
      // Auto-assign staggered delay to sibling cards if no explicit delay set
      if (!el.dataset.delay && !el.dataset.animateDelay) {
        var parent = el.parentNode;
        if (parent) {
          var siblings = parent.querySelectorAll(animationSelectors.join(', '));
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "קליק פדגוגי משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '17f38521-e4bc-4e6c-9db8-d21fbff71a17',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '17f38521-e4bc-4e6c-9db8-d21fbff71a17';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '17f38521-e4bc-4e6c-9db8-d21fbff71a17';
